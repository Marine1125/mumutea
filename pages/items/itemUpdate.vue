...
export default {
  ...
  async mounted() {
    const self = this
    const _id = self.$route.query._id
    await self.$axios
      ...
      .then(resp => {
        if (resp.status === 200) {
          if (resp.data && resp.data.code === 0) {
            //self.creator = resp.data.data.creator
    ...
    await self.$axios
      .get('/categorys/getCategoryList', self.itemForm)
      .then(resp => {
        if (resp.status === 200) {
          if (resp.data && resp.data.code === 0) {
  ...
  methods: {
    ...
    createItem: function() {
      ...
      self.$refs['itemForm'].validate(valid => {
        if (valid) {
          self.$axios.post('/items/updateItem', self.itemForm).then(resp => {
            if (resp.status === 200) {
              if (resp.data && resp.data.code === 0) {
                self.$message({
                  message: '创建成功',
